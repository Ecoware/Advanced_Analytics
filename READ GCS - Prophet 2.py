...
def ReadFromBQ():
    ...
    query = (
        'SELECT Timestamp, bytes FROM `ipfix_samples.ipfix_sample_1` '
        'WHERE bytes = "41" '
        'LIMIT 100')
...
#%%
def ReadFromGCS2(date):
    global proxy_juniper_data
    storage_client = storage.Client(project='network-sec-analytics')
    bucket = storage_client.get_bucket('juniper_logs')
    blob = bucket.get_blob(date)
    
    ## This creates a csv locally
    blob.download_to_filename('/home/steve/Data/RawData/'+ date +"'")
    
    
    ## Read a local file into a Dataframe
    proxy_juniper_data= pd.DataFrame() 
    proxy_juniper_data = pd.read_csv('/home/steve/Data/RawData/'+ date + "'",sep=',')  
    proxy_juniper_data.info()
    return()

ReadFromGCS2('6_07_2018')
...
proxy_juniper_data['date_hour'] = proxy_juniper_data.TIMESTAMP.str.slice(start=0,stop=13,step=None) 
proxy_juniper_data.sort_values(by=['date_hour','source_address','destination_address','destination_port','protocol_id'], ascending=False, inplace=True)

## THIS IS A COUNT DISTINCT ON THE 5 TUPLES
unq_rel_cnts = proxy_juniper_data.groupby(['date_hour','source_address','destination_address','destination_port','protocol_id']).size().reset_index(name='distinct_freq')
unq_rel_cnts.sort_values(by=['date_hour','source_address','destination_address','destination_port','protocol_id'], ascending=True, inplace=True)
...
#%%
unq_rel_cnts.sort_values(by=['date_hour','source_address','destination_address'], ascending=False, inplace=True)
