...
## add date_hour to the df
proxy_juniper_5_10_23['date_hour'] = proxy_juniper_5_10_23.datetime.str.slice(start=0,stop=13,step=None) 
#proxy_juniper_5_10_23.info()
#%%
### Count the number of times each unique dstAd was contacted by the Proxy Server.  
## THIS IS NOT A COUNT DISTINCT.
### Sort by Count in Descending order.  This enables the Rank to create the arbitary percentile groups (e.g., Deciles)
unq_rel_cnts = proxy_juniper_5_10_23.groupby(['date_hour','source_address','destination_address']).size().reset_index(name='distinct_freq')
unq_rel_cnts.sort_values(by='distinct_freq', ascending=False, inplace=True)

#unq_rel_cnts.sort_values(by='date_hour', ascending=False, inplace=True)
